...
class RawDataset:
    ...
    def create_feature_map_legend(self):
        ...
        for key, value in self.prepared_data.items():
            for attributes in value:
                if attributes[0] in [""]:
                    print("")
                    # check or replace wrong formatted data here
    ...
    def get_geo_locations_for_attribute_values(self, geo_cache_name, use_cache=True, check_zeros=False):
        ...
        for key, value in self.prepared_data.items():
            for attributes in value:
                if attributes[0] in ["address1_merged", "address2_merged", "address3_merged", "county", "continent",
                                     "country", "state"]:
                    look_up_place = attributes[1]
    ...
    async def api_request(places):
        ...
        futures = [
            loop.run_in_executor(
                None,
                requests.get,
                api_endpoint + place[2] + "&key=AIzaSyD3TjYgcFTNlYzmDLvZkv9XTLuTTNYwUdo"
            )
            for place in places  # islice(places, 0, 500) for fetching only 2500 per time
        ]
...
if __name__ == "__main__":
    ...
    dataset = RawDataset()
    dataset.load_data('electives', select_random=False)
    dataset.load_attributes('electives.attributes')
    dataset.load_geo_data('electives.geo')
    ...
    # dataset.get_geo_locations_for_attribute_values("electives", use_cache=True, check_zeros=False)

    duplicate_dataset_name = 'elective_representative'
