    ...
    const showAttributeComparison = () => {
        if (selectedNodes.length > 0) {
            ...
            if (options.attributeComparison.showOnlyAvailableAttributes) {
                attributes = availableItems.datasets[dataset].attributes[selectedFeatureSet].filter(value => -1 !== attributes.indexOf(value));
            } else {
                attributes = availableItems.datasets[dataset].attributes[selectedFeatureSet];
            }
